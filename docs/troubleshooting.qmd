...
From the repository root, rerun the canonical installation command. It refreshes
the collections declared in `requirements.yml` before running the playbook:
...
Resolve findings by adding deliberate workstation packages to Ansible-managed package lists, exact allowlists, or narrowly targeted regex allowlists. See [Audit Configuration](update_software.html) for the decision process.
