...
- **Tagged or other detached checkout:** `make update` skips the Git pull and
  ...
  updating Fedora and Flatpak software, refreshing dependencies, reapplying the
...
# Bootstrap prerequisites, refresh pinned Galaxy collections, and reapply the configuration
make install
...
2. Update Fedora packages and Flatpak applications from their configured repositories. These commands omit `-y` so you can review consequential changes before installation.
3. Run `make install` to ensure the bootstrap packages are present, refresh the
   pinned Galaxy collections from `requirements.yml`, and reapply the Ansible
   desired state.
4. Audit the resulting system for DNF package drift and high-signal manual filesystem installations.
