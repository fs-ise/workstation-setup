...
It is a self-contained shared lab baseline. A separate [personal overlay](personal_overlays.html)
may depend on and extend it, but this repository never depends on an overlay.
...
Executable Ansible entry points. `playbooks/lab-stack.yml` configures Fedora
Workstation, `playbooks/cloud-stack.yml` configures an existing server-oriented (non-GUI) Fedora
Cloud/server machine, and `playbooks/audit-unmanaged-packages.yml` audits package
drift. Cloud infrastructure provisioning remains outside this repository.
...
1. Follow the New machine pages in `docs/` to install Fedora and clone this repository.
2. Run `make configure`. It installs the bootstrap packages and pinned Galaxy
   collections before calling the main Ansible playbook through `lab-stack`.
3. The playbook applies focused roles from `roles/`.
4. Use Software pages for role-specific manual steps and checks.
5. Use Day-to-day operations pages for recurring [configuration updates](install_software.html#update-and-reapply-configuration), [audits](update_software.html), and [backup and sync](backup.html) routines.

## Best practices and useful links

- [Install OS](install_os.html)
