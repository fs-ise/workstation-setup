...
This page describes the intended roles of GitHub, Nextcloud, and HDD backups. The exact folder names can vary by workstation or lab, so treat examples such as `~/repos`, `~/Nextcloud`, and `~/workstation` as conventions to confirm locally.

## What belongs where

- **GitHub or another Git remote:** source code, text-based research projects, documentation, and versioned configuration that can be shared or made private as needed.
- **Nextcloud:** documents, PDFs, media, shared project folders, and files that should synchronize across devices but are not good Git content.
- **HDD backup with Vorta/Borg:** versioned encrypted backup of local home-folder data, including Git repositories, Nextcloud content, and local-only workstation folders.
