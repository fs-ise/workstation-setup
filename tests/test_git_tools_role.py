...
class Tests(unittest.TestCase):
 def test_packages_and_config(self):
  [self.assertIn(x,D) for x in ('git-lfs','gitk','log.date','push.autoSetupRemote','init.defaultBranch')]
  self.assertNotIn('core.editor',D); self.assertIn('community.general.git_config',T); self.assertIn('become_user: "{{ target_user }}"',T); self.assertIn('git_tools_packages | length > 0',T)
